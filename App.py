...
def extract_pdf_data(pdf_bytes, password, want_mobile):
    ...
    # Gender
    if "MALE" in text.upper() and "FEMALE" not in text.upper(): data["gender"] = "पुरुष/ MALE"
    elif "FEMALE" in text.upper(): data["gender"] = "महिला/ FEMALE"
    ...
    if issue_match: data["issue_date"] = issue_match.group(1)
    
    # Mobile Number (If requested)
    if want_mobile == 'yes':
        mob_match = re.search(r'[6-9][0-9]{9}', text.replace(" ", ""))
        if mob_match: data["mobile"] = mob_match.group(0)
...
def generate_card():
    try:
        ...
        # 3. Load Fonts (Ensure fonts are uploaded in GitHub repo)
        try:
            font_eng = ImageFont.truetype("times.ttf", 24)
            font_eng_bold = ImageFont.truetype("timesbd.ttf", 26)
            font_aadhaar = ImageFont.truetype("timesbd.ttf", 45)
            font_hi = ImageFont.truetype("mangal.ttf", 28) 
        ...
        # Agar sirf English name hai to spacing thoda badha denge
        ...
        # Template ki width estimate 1016px maankar center = 508
